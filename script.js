  ...
  cards.forEach(card => {
    gameBoard.insertAdjacentHTML('beforeend', `
      <div class="card" data-id="${card.id}">
        <div class="front" style="background-image: url(${cardBackImg});">
          <div class="dev-cheat"></div>
        </div>
        <div class="back" style="background-image: url(${cardBackImg});"></div>
      </div>
    `);
  });
...
async function startGame() {
  lockBoard = true;
  resetGameVariables();
  gameStatus.innerHTML = `<h3>Fetching Images...</h3><p id="fetch-progress">0/${boardSize * boardSize / 2} completed</p>`;
  gameStatus.classList.remove('hidden');
